import React, { useCallback } from 'react';

// import api from '../../services/api';
import apiIntegracao from '../../services/apiIntegracaoImoveis';

const BtnGetAll = () => {
  const acessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..31Px5-gjnIzQDaEHsAmxs71rXmeijg75fOgk9jzd9UQ';

  const printRequest = useCallback(
    async () => {
      await apiIntegracao.get('Imovel/obter-lista')
        .then((resp) => console.log(resp));
    }, [acessToken],
  );

  const getByUsername = useCallback(
    async (username, token) => {
      await apiIntegracao.user.getByUserName(username, token)
        .then((resp) => console.log('btn', resp));
    }, [],
  );

  return (
    <button type="button" onClick={() => getByUsername('victor', 'toknzinmaroto')}>GetAll</button>
  );
};

export default BtnGetAll;
